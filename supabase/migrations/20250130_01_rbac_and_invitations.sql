...
-- Skip audit_logs policy drop as table might not exist yet
...
-- Skip transfer_files policies as table might not exist yet
DROP POLICY IF EXISTS "Users can update their own profile" ON user_profiles;

-- Drop policies on company_join_requests that depend on company_members.role
DROP POLICY IF EXISTS "Company admins can view join requests" ON company_join_requests;
DROP POLICY IF EXISTS "Company admins can approve or reject join requests" ON company_join_requests;
DROP POLICY IF EXISTS "Company admins can update join requests" ON company_join_requests;
DROP POLICY IF EXISTS "Users can view their own join requests" ON company_join_requests;
DROP POLICY IF EXISTS "Users can update their own join requests" ON company_join_requests;

-- Drop all policies on company_members before altering column type
...
DROP POLICY IF EXISTS "Company members can update their own membership" ON company_members;
...
-- Drop project_members policies that depend on company_members.role
...
DROP POLICY IF EXISTS "Company admins can manage project members" ON project_members;
...
-- Skip audit_logs policy recreation as table might not exist yet
...
-- Skip transfer_files policies recreation as table doesn't exist yet

-- Step 15: Recreate company_join_requests policies with new role types
CREATE POLICY "Company admins can view join requests"
  ON company_join_requests FOR SELECT
  USING (
    auth.uid() IN (
      SELECT user_id FROM company_members 
      WHERE company_id = company_join_requests.company_id 
      AND role IN ('owner', 'admin')
    )
  );

CREATE POLICY "Company admins can approve or reject join requests"
...
-- Project members policies already recreated above